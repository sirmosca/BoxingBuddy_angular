            ...
            function BoxingMatchViewController(Settings, Combination, Speech) {
                ...
                self.startBoxing = function() {
                    ...
                    self.startCountdown().then(() => {
                        ...
                        self.executeRound(round, roundTime, maxRounds, roundIntermission, timeBetweenPunches, timeBetweenCombos);
                    });
                };

                self.executeRound = function(currentRound, roundTime, maxRounds, roundIntermission, timeBetweenPunches, timeBetweenCombos) {
                    self.pauseBoxing = false;
                    self.doPunches(timeBetweenPunches, timeBetweenCombos);
                    return Settings.sleep(() => {
                        self.stopPunching();
                    }, roundTime)
                    .then(() => { return self.roundIntermission(roundIntermission) })
                    .then(() => {
                        if (currentRound === maxRounds || self.startedBoxing === false) {
                            ...
                            return self.executeRound(currentRound+1, roundTime, maxRounds, roundIntermission, timeBetweenPunches, timeBetweenCombos);
                        }
                    });
                };
                ...
                self.doPunches = function(timeBetweenPunches, timeBetweenCombos) {
                    ...
                    self.throwCombo(timeBetweenPunches, timeBetweenCombos)
                        .then(() => {
                            Settings.sleep(() => {self.doPunches(timeBetweenPunches, timeBetweenCombos)}, timeBetweenCombos);
                        });
                };
                ...
                self.throwCombo = function(timeBetweenPunches, timeBetweenCombos) {
                    ...
                    const interval = self.combos[comboIndex].punches.length * timeBetweenPunches;
                ...
                self.startCountdown = function() {
                    ...
                    return Settings.sleep(start, 0)
                        .then(() => {return Settings.sleep(two, 1000)})
                        .then(() => {return Settings.sleep(one,1000)})
                        .then(() => {return Settings.sleep(() => {
                            showPunchImg();
                            Speech.sayText("Fight");
                            self.changeDisplay("");
                        }, 1000)})
                        .then(() => {return Settings.sleep(hidePunchImg, 0)})
                        .then(() => {return Settings.sleep(() => {}, 250)});
                };
